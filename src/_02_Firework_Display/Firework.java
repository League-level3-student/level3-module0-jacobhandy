...
public class Firework{
	...
	public Firework() {
		...
		for(int a = 0; a < spark.length; a++) {
			spark[a] = new Spark(750,750);
		}
	}
	...
	public void drawSparks(Graphics g) {
		...
		for(int a = 0; a < spark.length; a++) {
		  g.setColor(spark[a].color);
		  g.fillOval(spark[a].x, spark[a].y, spark[a].size, spark[a].size);
		}
